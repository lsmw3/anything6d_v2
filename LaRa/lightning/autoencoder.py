...
class BetaVAE(nn.Module):
    ...
    def __init__(self,
        ...
        for h_dim in hidden_dims:
            modules.append(
                nn.Sequential(
                    nn.Conv2d(in_channels, out_channels=h_dim, kernel_size=3, stride=2, padding=1,groups=3),
        ...
        self.fc_mu = nn.Linear(hidden_dims[-1], latent_dim)  # for plane features resolution 64x64, spatial resolution is 2x2 after the last encoder layer
    ...
    def decode(self, z: Tensor) -> Tensor:
        ...
        '''
        # print(z.norm())
        result = self.decoder_input(z) # ([B, D*4])
        result = result.unsqueeze(-1).unsqueeze(-1)  # for plane features resolution 64x64, spatial resolution is 2x2 after the last encoder layer
        result = self.decoder(result)
        result = self.final_layer(result) # ([B, 3, resolution, resolution])
    ...
    def reparameterize(self, mu: Tensor, logvar: Tensor) -> Tensor:
        ...
        :return:
        """
        ones = torch.ones_like(mu).to(mu.device)
    ...
    def forward(self, data: Tensor, **kwargs) -> Tensor:
        ...
        # print(z.mean())
        # print(z.std())
        #z = self.reparameterize(mu, torch.Tensor(0.001).to(mu.device)) 
    ...
    def loss_function(self,
        ...
        return kld_weight * kl_loss + recons_loss, kl_loss, recons_loss
