...
def main(cfg):
    ...
    for i in tqdm(range(len(loader))):#len(loader)
        ...
        output = my_system.net(sample, with_fine=True, return_buffer=return_buffer) # True
        
        name = sample['meta']['scene'][0].split('.')[0]
        images = output['image_fine'][0] #'image_fine'
        img_gt = sample['tar_rgb'][0].permute(1,0,2,3).reshape(images.shape)
        alpha = output['acc_map'][0][...,None]
        normal_white = ((output['rend_normal_fine'][0]*alpha+1-alpha) + 1)/2 # 'rend_normal_fine'
        ...
        if len(cfg.infer.eval_depth):
            ...
            depth_pred = output['depth_fine'].cpu().squeeze(-1).detach().numpy() # 'depth_fine'
        ...
        if cfg.infer.video_frames > 0:
            ...
            gs_params = output['render_pkg'][1] # fine ouputs 1
            ...
            imageio.mimwrite(f'{cfg.infer.save_folder}/feedforward_test.mp4', imgs, format='ffmpeg', **output_options)
            imageio.mimwrite(f'{cfg.infer.save_folder}/nrm_feedforward_test.mp4', normal_whites, format='ffmpeg', **output_options)
        ...
        if cfg.infer.save_mesh:
            ...
            meshExtractor.extract(f'{cfg.infer.save_folder}/feedforward_test.obj', cfg.infer.dataset, sample=sample, fov=fov)
