...
gpu_id = 0
name = 'release'
ckpt_path = "ckpts/epoch=29.ckpt" # f'ckpts/epoch=29.ckpt'
...
for n_views in [4]:
    ...
    #     f'infer.dataset.dataset_name=gobjeverse infer.dataset.data_root=/mnt/hdd/lara_data/dataset/gobjaverse/gobjaverse.h5 ' \
    #     f'infer.video_frames=0 infer.save_mesh=False ' \
    #     f'infer.save_folder=outputs/image_vis/{name}_gobjaverse_{n_views}_views infer.dataset.n_group={n_views} '
    # os.system(cmd)

    cmd = f'CUDA_VISIBLE_DEVICES={gpu_id} python evaluation.py configs/infer.yaml n_views={n_views} infer.eval_novel_view_only=True ' \
        f'infer.ckpt_path={ckpt_path} infer.metric_path=outputs/metrics/{name}_feedforward_test_{n_views}_views.json  ' \
        f'infer.dataset.dataset_name=gobjeverse infer.dataset.data_root=/mnt/hdd/lara_data/dataset/Co3D/co3d_teddybear.h5 ' \
        f'infer.video_frames=120 infer.save_mesh=True ' \
        f'infer.save_folder=outputs/image_vis/housecat_instance infer.dataset.n_group={n_views} '
    os.system(cmd)
